...
| [`release.yml`](.github/workflows/release.yml) | Shared release flow (needs a human): it accepts the tag as the sole release coordinate, validates the committed preparation receipt, extracts the matching CHANGELOG section, creates + pushes the tag, and publishes the GitHub Release as `three-cubes-agent[bot]`. Repo-specific checks stay in the caller. The caller provides WIF and `AZURE_*` repository variables. |
| [`prepare-release-metadata`](actions/prepare-release-metadata/action.yml) | Composite action for the reviewed release-preparation branch: promotes populated `Unreleased` notes to the tag-derived section, writes the version source, and records the receipt `release.yml` requires before it can cut a tag. |
