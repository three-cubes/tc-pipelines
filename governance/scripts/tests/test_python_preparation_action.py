...
def test_consumer_ruff_config_is_a_fixed_point_without_executing_project_code(
    ...
    subprocess.run(["git", "add", "pyproject.toml", "alias.py", "formatting.py"], cwd=repo, check=True)
    environment = {
        **os.environ,
        "GITHUB_ACTION_PATH": str(ROOT / "actions/python-preparation"),
        "UV_VERSION": "",
    ...
    first = subprocess.run(["bash", "-euo", "pipefail", "-c", script], cwd=repo, env=environment, check=False)
    ...
    settings = subprocess.run(
        ["uvx", "--from", "ruff==0.16.8", "ruff", "check", "--show-settings", "alias.py"],
        cwd=repo,
        check=False,
        capture_output=True,
        text=True,
...
def test_nested_ruff_config_does_not_change_unconfigured_sibling_defaults(tmp_path: Path) -> None:
    document = yaml.safe_load((ROOT / "actions/python-preparation/action.yml").read_text())
    script = document["runs"]["steps"][0]["run"]
...
def test_requires_python_infers_ruff_target_without_forcing_shared_defaults(tmp_path: Path) -> None:
    document = yaml.safe_load((ROOT / "actions/python-preparation/action.yml").read_text())
    script = document["runs"]["steps"][0]["run"]
