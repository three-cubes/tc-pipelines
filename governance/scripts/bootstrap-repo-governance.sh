...
#     [--dry-run]
#     [--render-only]  # deterministic local files only; no gh/az lookups
#
...
# Local assurance and bootstrap previews use the same renderer without org
# discovery, credentials, remote writes or environment-dependent secret scans.
...
if [[ "$DO_WIRING" == "1" ]]; then
  ...
  if [[ -z "$SKELETON_DIR" ]]; then
    ...
    if [[ "$RENDER_ONLY" == "0" ]] && command -v detect-secrets >/dev/null 2>&1; then
