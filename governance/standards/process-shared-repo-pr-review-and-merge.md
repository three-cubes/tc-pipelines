...
`auto-merge.yml` records `gh pr merge --auto --merge` as the App when the PR opens or changes. The workflow runs from the default branch, reads PR metadata, and never checks out pull-request content. GitHub completes the merge after the required human review and contexts pass: **Quality gate** + **SonarCloud scan** + **SonarCloud Code Analysis** (a hardened gate also requires **Mutation**). Use the merge-commit method, never squash.
