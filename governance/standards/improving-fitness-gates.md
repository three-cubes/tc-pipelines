...
3. **Tag the change**, then **bump this repo's own self-pins to that tag and merge it**, then tag again. A self-pin cannot name the tag being cut, so a change to a composite reaches consumers only at the second tag — and leaving `main` standing on a tag with stale pins turns every pin assertion red on whoever's PR lands next. The full rule, including why the SHAs are repeated rather than named once and what a consumer repin actually touches, is [`supply-chain-pinning.md`](supply-chain-pinning.md).
