...
| **Review → Merge** | `auto-merge.yml` records `gh pr merge --auto` as the App when the PR opens or changes. GitHub merges after every required check and review pass. Branch deletes after merge. |
...
**Ordinary work auto-merges through branch rules.** `auto-merge.yml` runs from the default branch on PR metadata events and records `gh pr merge --auto` as the App. GitHub merges when every required context and review passes. The job reads metadata only; it does not check out or execute pull-request content.
