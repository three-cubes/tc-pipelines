...
|---|---|---|
| [`rulesets/main-product.json`](rulesets/main-product.json) (consumer repos) + [`rulesets/main-core.json`](rulesets/main-core.json) (paved-path repos) | The `main` ruleset snapshots. See [`CANONICAL-ORG-RULESET.md`](CANONICAL-ORG-RULESET.md) for the required checks and review rules (single source of truth). | `gh api repos/<repo>/rulesets --method POST` |
...
produces the required `Quality gate` check lives here in
[tc-pipelines](../README.md). (The required-check set is defined in
[`CANONICAL-ORG-RULESET.md`](CANONICAL-ORG-RULESET.md) — the single source of
truth; SonarCloud is no longer a required context.)
