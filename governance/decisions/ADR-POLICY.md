...
| `RULESET-D1` | The `main` ruleset: block deletion + force-push; PR required with code-owner review on control-plane paths; **not strict** (no forced rebase) + **no stale-dismiss**; two profiles of the one standard — product (autonomous, 0 approvals) and paved-path core (1 approval). See [`CANONICAL-ORG-RULESET.md`](../CANONICAL-ORG-RULESET.md) for the required checks and review rules (single source of truth). | [`CANONICAL-ORG-RULESET.md`](../CANONICAL-ORG-RULESET.md); profiles [`rulesets/main-product.json`](../rulesets/main-product.json) + [`rulesets/main-core.json`](../rulesets/main-core.json); `governance/README.md` |
