...
| F8 | `tc-fitness` `0.17.1` runs once as an independently executable, repository-scoped graph task and a version mismatch is rejected. | VERIFIED | Fitness component implementation at `062e16e`; package suite 153/153, packed public consumer/parity/generator suite 21/21 and full repository gate 1,991/1,991 passed. Independent review reproduced terminal receipt creation for unreadable input and passed the public command. Sabotage covers hostile `PATH`, version mismatch, missing or ambiguous distribution metadata, changed or missing executable, missing declaration and malformed catalogue or lock. | None. |
...
| F16 | The consolidated branch passes the repository's complete local admission path with a clean worktree. | IN PROGRESS | F1–F8 have passing behavioural evidence on independently reviewed component commits. | Integrate F9–F15, then pass build, package tests, Darwin integration, Docker/image qualification, Python assurance, actionlint, diff check, `make check` and `make assurance all` on the exact head. |
...
are independently executable and verified through F8. Consumer qualification,
