...
| F16 | The consolidated branch passes the repository's complete local admission path with a clean worktree. | IN PROGRESS | F1–F6 have passing focused, package, Darwin and Docker evidence on their reviewed component commits. | Integrate F7–F15, then pass build, package tests, Darwin integration, Docker/image qualification, Python assurance, actionlint, diff check, `make check` and `make assurance all` on the exact head. |
