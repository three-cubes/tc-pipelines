...
| F16 | The consolidated branch passes the repository's complete local admission path with a clean worktree. | IN PROGRESS | F1–F7 have passing behavioural evidence on independently reviewed component commits. | Integrate F8–F15, then pass build, package tests, Darwin integration, Docker/image qualification, Python assurance, actionlint, diff check, `make check` and `make assurance all` on the exact head. |
